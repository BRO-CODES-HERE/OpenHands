...
## 2024-10-25 - Path Traversal Vulnerability in Tools
**Vulnerability:** System tools like `read_file`, `write_file`, and `list_dir` used user input to construct file paths directly with `path.resolve(args.path)`. This allowed an agent (or malicious actor sending commands to the agent) to escape the workspace via path traversal (e.g. `../../../etc/passwd` or `/tmp/secret`) and read/write arbitrary files on the filesystem.
**Learning:** Using raw `path.resolve` is insufficient for containing file operations within a specific root directory. It blindly resolves whatever relative or absolute path is provided, leading to path traversal outside of intended boundaries.
**Prevention:** Implement a utility like `resolveSafePath` that resolves the absolute path based on the root directory (`process.cwd()`), computes the relative path to the root directory, and asserts that the relative path does not start with `..` and is not absolute, effectively confining all operations to the workspace.
