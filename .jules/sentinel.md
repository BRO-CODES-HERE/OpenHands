...
## 2024-06-25 - Path Traversal Vulnerability in System Tools
**Vulnerability:** The system tools (`ReadFileTool`, `WriteFileTool`, `ListDirTool`) used `path.resolve` directly on user-provided paths without verifying if the resolved path remained within the workspace directory boundaries. This allowed agents or users to potentially read, overwrite, or list sensitive files on the host filesystem (e.g., `/etc/passwd` or outside the project root) by supplying paths like `../../../etc/passwd`.
**Learning:** Functions dealing with the filesystem that construct paths from user input must ensure the inputs cannot traverse directory bounds. Relying solely on `path.resolve` is insufficient because it resolves absolute paths directly and allows `..` sequences to escape the intended root.
**Prevention:** Always validate and sanitize user input before using it in file paths. Implement a helper function (e.g., `resolveSafePath`) that compares the relative path from the workspace directory to the resolved path and rejects any path that starts with `..` or is an absolute path relative to the workspace.
