...
## 2024-06-13 - [CRITICAL] Prevent API Key Exposure over Configuration WebSockets
**Vulnerability:** The `config.get` WebSocket endpoint previously transmitted the entire application configuration payload in plaintext, exposing sensitive secrets like LLM provider API keys to any connected client. Since local WebSocket APIs can sometimes be interrogated directly or sniffed by tools, returning plain API keys is a significant risk.
**Learning:** Returning unmasked secrets from API endpoints, even if they are internal configuration getters intended to populate frontend UIs, represents a major data leak. Providing a mechanism to mask (`********`) keys on egress prevents network sniffing, but creates a challenge for `set` operations because blindly saving back the masked value would corrupt the stored secrets.
**Prevention:** Always mask or strip sensitive keys (like API tokens and passwords) on egress APIs (e.g. `config.get`). On ingress APIs (`config.set`), verify if the incoming secret matches the masked signature (e.g., `********`) and, if so, retrieve the prior state to restore the genuine token before writing to persistent storage.
