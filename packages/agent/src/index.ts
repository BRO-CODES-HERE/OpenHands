  ...
  public async run(userMessage: string, maxIterations: number = 10): Promise<string> {
    ...
    for (let i = 0; i < maxIterations; i++) {
      ...
      if (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
        // ⚡ Bolt: Execute tool calls concurrently using Promise.all to improve performance
        // This reduces wait time for multiple slow tools from O(N) to O(1) in terms of I/O.
